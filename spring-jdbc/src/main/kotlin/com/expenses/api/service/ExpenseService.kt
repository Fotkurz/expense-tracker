...
import com.expenses.api.domain.CustomExpendedAtFilter
...
class ExpenseService(
    ...
    private fun findAllFilteredByExpendedAt(request: FindExpensesRequest): Page<Expense> {
        ...
        try {
            val paginated = when (ExpendedAtFilter.valueOf(expendedAt)) {
                ExpendedAtFilter.PAST_WEEK -> repository.findByExpendedAtBetween(
                    ZonedDateTime.now().minusWeeks(1),
                    ZonedDateTime.now(), request
                )

                ExpendedAtFilter.PAST_MONTH -> repository.findByExpendedAtBetween(
                    ZonedDateTime.now().minusMonths(1),
                    ZonedDateTime.now(), request
                )

                ExpendedAtFilter.PAST_3_MONTH -> repository.findByExpendedAtBetween(
            ...
            val filter = CustomExpendedAtFilter(expendedAt)
            val found = repository.findByExpendedAtBetween(filter.from, filter.to, request)
            return Page(
